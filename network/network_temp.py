...
def run(g_syn_inh, g_syn_exc, T):
    """ Runs the neuronal network
    Params:
    g_syn_inh: g_syn for inhibitory neurons
    g_syn_exc: g_syn for excitatory neurons
    ...
    for t in np.arange(start_time, end_time, inter_time):
        ...
        for ni, I_syn in zip(range(n_neu), neu_I_syn_db[-1]):
            ...
            V_, m_, h_, n_ = uneu.run(t, V, m, h, n, 
                                      I_ext=I_syn+I_app, T=T)
        ...
        for pre_ni in range(n_neu):
            ...
            for si in syn_pre_dict[pre_ni]:
                ...
                s_, I_syn_ = usyn.run(t, V_pre=V_pre, s=s, typ=typ, g=g, T=T)
    ...
    plt.title(
        r'\textbf{LFP, $g_{inh} = $%.4f, $g_{exc} = $%.4f, $T = $%.2f}' % 
        (g_syn_inh, g_syn_exc, T))
    plt.plot(x, y)
    plt.ylabel(r'\textbf{Membrane potential} (mV)')
    plt.xlabel(r'\textbf{Time} (ms)')
    # plt.show()

    f_name = 'LFP_p%d_e%d_s%d_f%d_%.1fms_gin%.2f_gex%.2f_T%d.png' %\
            (n_pyr, n_exc, n_sin, n_fin, 
             end_time, g_syn_inh, g_syn_exc, T)
...
if __name__ == '__main__':
    ...
    print "-temperature:", ops.T

    run(g_syn_inh=ops.gin, g_syn_exc=ops.gex, T=ops.T)
